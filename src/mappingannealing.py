...
function: mapping_annealing(netlist_path, cost_estimator_path, library_path, output_path, 
                            predefined_gates_dict (if has predifined assignment dictionary))
...
function: abc_mapping_annealing(netlist_path (generated by ABC), cost_estimator_path, library_path, output_path,
                                predefined_gates_dict (if has predifined assignment dictionary))
this function takes in the path to the netlist file, the path to the cost estimator, 
the path to the library file, and the path to the output file.
returns the final cost after doing simulated annealing algorithm.
...
def mapping_annealing(netlist_path, cost_estimator_path, library_path, output_path, predefined_gates_dict = None):
    ...
    gate_number_result = []
    if predefined_gates_dict == None:
        for gate in gates:
            gate_number_result.append(random.randint(1,typesofgate[gate[0]]))
    else:
        for gate in gates:
            gate_number_result.append(predefined_gates_dict[gate[0]])
...
def abc_mapping_annealing(netlist_path, cost_estimator_path, library_path, output_path, predefined_gates_dict = None):
    # read the verilog file
...
if __name__ == "__main__":
    ...
    # dictt = {'and': 1, 'or': 1, 'xor': 1, 'nand': 1, 'nor': 1, 'xnor': 1, 'not': 1, 'buf': 1}
    # mapping_annealing(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], dictt)
